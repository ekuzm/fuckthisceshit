...
static void enable_autostart(Logic& app) {
    ...
    if (g_mkdir_with_parents(dir.c_str(), 0700) != 0) {
        report_error(app, "Не удалось создать каталог автозапуска: " + std::string(std::strerror(errno)));
        return;
    }
    ...
    if (!g_key_file_save_to_file(entry, path.c_str(), &error)) {
        ...
        g_clear_error(&error);
    } else log(app, Kind::System, "Автозапуск включён: " + path);
...
void set_autostart(Logic& app, bool enabled) {
    if (enabled) enable_autostart(app);
    else disable_autostart(app);
}
