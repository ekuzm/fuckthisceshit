...
static void log_power_changes(Logic& app, const PowerState& next) {
    if (!next.error.empty() && next.error != app.state.error) log(app, Kind::System, next.error);
    if (next.error.empty() && !app.state.error.empty()) log(app, Kind::System, "Чтение sysfs восстановлено");
    ...
    if (!app.initialized && next.batteries.empty()) log(app, Kind::System, "Батарея не обнаружена");
}
...
void refresh(Logic& app) {
    ...
    if (app.on_state_changed) app.on_state_changed();
}
