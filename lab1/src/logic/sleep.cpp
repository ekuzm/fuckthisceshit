...
void request_power_action(Logic& app, const char* action) {
    ...
    auto* process = g_subprocess_new(static_cast<GSubprocessFlags>(G_SUBPROCESS_FLAGS_STDOUT_SILENCE |
        G_SUBPROCESS_FLAGS_STDERR_PIPE), &error, "systemctl", action, nullptr);
    if (!process) {
