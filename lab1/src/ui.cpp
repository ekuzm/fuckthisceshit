...
void render_log(App& app) {
    ...
    for (const auto& event : app.logic.events)
        if (selected == Kind::All || selected == event.kind) text += event.line + '\n';
...
void label(cairo_t* cr, const std::string& text, double y, int size, bool muted = false) {
    ...
    if (muted) cairo_set_source_rgb(cr, 0.65, 0.71, 0.80);
    else cairo_set_source_rgb(cr, 0.94, 0.96, 1.0);
...
void draw_charge_ring(cairo_t* cr, double value) {
    ...
    cairo_stroke(cr);
    if (value >= 0) {
        if (value >= 50) cairo_set_source_rgb(cr, 0.20, 0.83, 0.60);
        else if (value >= 20) cairo_set_source_rgb(cr, 0.98, 0.76, 0.20);
        else cairo_set_source_rgb(cr, 0.98, 0.33, 0.36);
...
std::string battery_detail(const Battery* battery) {
    if (!battery) return "Данные батареи недоступны";
    if (battery->status != "Discharging") return status_name(battery->status);
    if (battery->minutes < 0) return "Оставшееся время неизвестно";
...
void show_error(App& app, const std::string& text) {
    auto* dialog = gtk_message_dialog_new(GTK_WINDOW(app.window), GTK_DIALOG_MODAL,
        GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, "%s", valid_utf8(text).c_str());
...
void on_save_log(GtkButton*, gpointer data) {
    auto& app = *static_cast<App*>(data);
    auto* dialog = gtk_file_chooser_dialog_new("Сохранить полный журнал", GTK_WINDOW(app.log_window),
        GTK_FILE_CHOOSER_ACTION_SAVE, "Отмена", GTK_RESPONSE_CANCEL,
        "Сохранить", GTK_RESPONSE_ACCEPT, nullptr);
...
void create_log_window(App& app) {
    ...
    g_signal_connect(app.log_window, "delete-event", G_CALLBACK(gtk_widget_hide_on_delete), nullptr);
    ...
    for (const char* text : {"Все события", "Подключение / отключение ЗУ", "Изменение заряда", "Переходы в сон"})
...
void show_log(GtkMenuItem*, gpointer data) {
    auto& app = *static_cast<App*>(data);
    if (!app.log_window) create_log_window(app);
...
void power_action(GtkMenuItem* item, gpointer data) {
    ...
    auto* dialog = gtk_message_dialog_new(GTK_WINDOW(app.window), GTK_DIALOG_MODAL,
        ...
        std::strcmp(action, "suspend") == 0 ? "Перевести компьютер в спящий режим?" : "Перевести компьютер в гибернацию?");
...
void add_battery_menu(App& app) {
    if (app.logic.state.batteries.size() <= 1) return;
    ...
    for (const auto& battery : app.logic.state.batteries) {
        auto* item = gtk_menu_item_new_with_label((battery.name + (battery.name == app.selected ? " ✓" : "")).c_str());
...
gboolean mouse(GtkWidget*, GdkEventButton* event, gpointer data) {
    ...
    if (event->button == GDK_BUTTON_PRIMARY) {
        gtk_window_begin_move_drag(GTK_WINDOW(app.window), event->button,
                                  static_cast<int>(event->x_root), static_cast<int>(event->y_root), event->time);
        return TRUE;
    }
...
void apply_theme() {
    ...
    gtk_css_provider_load_from_data(css,
        ...
        "button, combobox button { background-image: none; background-color: #29364a; color: #f0f5ff; }"
        "button:hover, menuitem:hover { background-color: #374b65; }", -1, nullptr);
...
int run_ui(int argc, char** argv) {
    ...
    app.logic.on_state_changed = [&app] { update_state(app); };
    app.logic.on_log_changed = [&app] { render_log(app); };
    app.logic.on_error = [&app](const std::string& text) { show_error(app, text); };
